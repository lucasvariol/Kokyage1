...
import { ownerConsentResumedTemplate } from '@/email-templates/owner-consent-resumed';
...
export async function POST(req) {
  try {
    ...
    // Set back to moderator pending review
    const newStatus = 'en attente validation modérateur';
    ...
    // Notifier le locataire principal (owner_id) que l'accord est réactivé
    let emailSent = false;
    if (listing.owner_id) {
      try {
        ...
        if (tenantUser?.email) {
          ...
          const emailPayload = {
            ...
            proprietaireName,
            resumedAt: new Date().toLocaleDateString('fr-FR', {
              day: 'numeric',
              month: 'long',
              year: 'numeric'
            })
          ...
          await resend.emails.send({
            ...
            to: tenantUser.email,
            subject: ownerConsentResumedTemplate.subject,
            html: ownerConsentResumedTemplate.getHtml(emailPayload),
            text: ownerConsentResumedTemplate.getText(emailPayload)
          });
          emailSent = true;
        } else {
          console.warn('resume route: tenant user has no email', { ownerId: listing.owner_id });
        }
      } catch (emailErr) {
        console.error('resume route: email send failed', emailErr);
      }
    }
