...
export function calculateShares(hebergementTotal, fraisPlateforme) {
  ...
  // fraisPlateforme est TTC (inclut la TVA)
  // On calcule le montant HT et la TVA séparément
  const fraisPlateformeHT = fraisPlateforme / (1 + VAT_RATE);
  const fraisPlateformeTVA = fraisPlateforme - fraisPlateformeHT;
  
  // platform_share = frais plateforme HT + commission sur hébergement (HT)
  const platform_share = fraisPlateformeHT + (hebergementTotal * HOST_COMMISSION);
  const platform_tva = fraisPlateformeTVA; // TVA uniquement sur les frais de plateforme
