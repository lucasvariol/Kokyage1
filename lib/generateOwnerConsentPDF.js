    ...
    segments.forEach(segment => {
      ...
      if (segIndex !== -1) {
        ...
        if (segment.type === 'bold') {
          ...
          // Mesurer en police bold pour éviter que le texte suivant se chevauche
          const boldWidth = doc.getTextWidth(segText);
          doc.setFont('helvetica', 'normal');
          currentX += boldWidth;
...
export async function generateOwnerConsentPDF(listingId) {
  try {
    ...
    // Important: réinitialiser la couleur du texte pour le contenu du document
    doc.setTextColor(0, 0, 0);
