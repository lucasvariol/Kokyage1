 ...
 */
function processMarkdownLine(doc, line, margin, y, maxWidth, pageWidth) {
  // Regex pour détecter **gras**, [lien](url)
  const segments = [];
  let currentIndex = 0;
  
  // Regex combinée pour gras et liens
  const regex = /(\*\*[^*]+\*\*|\[[^\]]+\]\([^)]+\))/g;
  let match;
  ...
  // Si pas de markdown détecté, traiter comme texte normal
  ...
  segments.forEach(segment => {
    ...
    if (x + textWidth > pageWidth - margin && x > margin) {
      x = margin;
      currentY += 5;
    ...
    if (segment.type === 'bold') {
      ...
      doc.setTextColor(215, 144, 119); // Couleur #D79077
      doc.textWithLink(text, x, currentY, { url: segment.url.startsWith('http') ? segment.url : `https://kokyage.com${segment.url}` });
...
export async function generateOwnerConsentPDF(listingId) {
  try {
    ...
    lines.forEach(line => {
      ...
      const processedText = processMarkdownLine(doc, trimmedLine, margin, y, maxWidth, pageWidth);
